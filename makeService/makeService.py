...
def parseInput():
    opts, args = getopt.getopt(
        sys.argv[1:], "hi:o:", longopts=["help", "input=", "output="]
    )
    ...
    if ".proto" not in proto_file:
        raise Exception(
            "Generate error, input file is't standard protobuf file:["
            + proto_file
            + "]"
        )
...
def printHelp():
    ...
    print(
        ("    ")
        + "Input the target proto file, must standard protobuf3 file, not support protobuf2.\n"
    )

    print("-o dir, --output dir")
    print(
        ("    ")
        + "Set the path that your want to generate project, please give a dir param.\n"
    )
...
def generate_dir():
    ...
    test_client_tool_path = test_client_path + "/test_tool_execut"
...
def generate_cmakelists():
    print("=" * 100)
    print("Begin to generate CMakeLists")
    ...
    template_file = open(generator_path + "/template/CMakeLists.template", "r")
...
def generate_cmakelists_test():
    print("=" * 100)
    print("Begin to generate cmakelists_test")
    out_file = test_client_path + "/CMakeLists.txt"
    if os.path.exists(out_file):
        print("cmakelists_test exist, skip generate")
        print("End generate CMakeLists")
        print(out_file)
        print("=" * 100)
        return

    template_file = open(generator_path + "/template/CMakeLists_test_template", "r")
    tmpl = Template(template_file.read())

    content = tmpl.safe_substitute(
        PROJECT_NAME=project_name + "_test_client",
        CREATE_TIME=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    )
    ...
    file.close()
    print("succ write to " + out_file)
    print("End generate cmakelists_test")
    print("=" * 100)
...
def gen_conf_file():
    print("=" * 100)
    file_name = "rpc.xml"
    print("Begin to generate mircorpc conf file")
...
def generate_framework_code():
    print("=" * 100)
    print("Begin to generate mircorpc framework code")
    ...
    if not os.path.exists(exception_file):
        ...
        exception_template = Template(
            open(generator_path + "/template/business_exception.h.template", "r").read()
        )
    ...
    head_file_temlate = Template(
        open(generator_path + "/template/server.h.template", "r").read()
    )
    head_file_content = head_file_temlate.safe_substitute(
        FILE_NAME=project_name + ".h",
        CREATE_TIME=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        INCLUDE_PB_HEADER='#include "' + "pb/" + project_name + '.pb.h"',
        PROJECT_NAME=project_name,
    ...
    cc_file_temlate = Template(
        open(generator_path + "/template/server.cc.template", "r").read()
    )
    cc_file_content = cc_file_temlate.safe_substitute(
        ...
        INCLUDE_BUSINESS_EXCEPTION_HEADER='#include "' + 'comm/business_exception.h"',
        PROJECT_NAME=project_name,
    )
    ...
    for each in interface_list:
        include_str += '#include "' + "interface/" + each["interface_name"] + '.h"\n'
    ...
    main_file = proj_path + "/main.cc"
    if not os.path.exists(main_file):
        main_file_temlate = Template(
            open(generator_path + "/template/main.cc.template", "r").read()
        )
    ...
    interface_head_file_temlate = Template(
        open(generator_path + "/template/interface.h.template", "r").read()
    )
    interface_cc_file_temlate = Template(
        open(generator_path + "/template/interface.cc.template", "r").read()
    )
    interface_test_client_file_template = Template(
        open(generator_path + "/template/test_rpc_client.cc.template", "r").read()
    )
    ...
    for each in interface_list:
        ...
        if not os.path.exists(file):
            cc_file_content = interface_cc_file_temlate.safe_substitute(
                PROJECT_NAME=project_name,
                INCLUDE_PB_HEADER='#include "' + "pb/" + project_name + '.pb.h"',
                INCLUDE_INTERFACE_HEADER_FILE='#include "'
                + "interface/"
                + each["interface_name"]
                + '.h"',
                CREATE_TIME=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
...
def generate_project():
    ...
    try:
        ...
        print("Succ generate mircorpc project")
        print("=" * 150)

    except Exception as e:
        print("Failed to generate mircorpc server, err: " + str(e))
        print("=" * 150)
